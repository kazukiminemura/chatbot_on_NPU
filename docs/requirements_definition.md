# Qwen2.5-1.5B NPUチャットボット アプリケーション要件定義書

## 1. プロジェクト概要

### 1.1 目的
- Qwen2.5-1.5B-Instruct-GGUFモデルをOpenVINOを使用してNPU上で実行
...
2. **モデル管理機能**
   - Qwen2.5-1.5B-Instruct-GGUFモデルの自動ダウンロード
   - OpenVINO形式への自動変換
   - NPUデバイスへの最適化配置
...
### 5.1 モデルデータ
- **モデルサイズ**: 約3GB (Qwen2.5-1.5B)
- **フォーマット**: GGUF → OpenVINO IR変換
