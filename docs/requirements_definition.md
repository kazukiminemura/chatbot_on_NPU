# Llama2-7B NPUチャットボット アプリケーション要件定義書

## 1. プロジェクト概要

### 1.1 目的
- Llama2-7BモデルをOpenVINOを使用してNPU上で実行
- ブラウザから利用可能な個人用チャットボットアプリケーションの構築
- 高品質な対話生成とリアルタイム応答の実現
...
2. **モデル管理機能**
   - Llama2-7BモデルのQuantized版の自動ダウンロード
   - OpenVINO形式への自動変換
   - NPUデバイスへの最適化配置
...
### 5.1 モデルデータ
- **モデルサイズ**: 約6.5GB (Llama2-7B Quantized)
